...
function normalizeVerifyOptions(providedOptions, config) {
    ...
    return {
        ...
        ruleFilter,
		only: providedOptions.only,
		except: providedOptions.except
    };
}
...
function _getFilteredRules(rules, options) {
	...
	{
		list = lodash.omitBy(list, function(value, key) {
...
function runRules(
	...
	configuredRules = _getFilteredRules(configuredRules, options);
...
// Lazy loaded type rules
let disabledTypeRules;

// Helper to initialize type rules only when needed
function getDisabledTypeRules() {
    if (!disabledTypeRules) {
        disabledTypeRules = [
    ...
    return disabledTypeRules;
}

// 1. disable type-checking
// 2. turn-off type-checked rules
function modifyConfigIfNeeded(config) {
	
    let modifiedConfig = config;
	
    if (config?.languageOptions?.parserOptions?.projectService != null) {
        modifiedConfig = {
            ...modifiedConfig,
            languageOptions: {
    ...
    if (config?.rules != null) {
        const rules = getDisabledTypeRules();
        
        // Only disable rules that exist in the config
        const disabledRules = {};
...
class Linter {
    ...
    #eslintrcVerifyWithoutProcessors(file, providedConfig, providedOptions) {
        ...
        const config = modifyConfigIfNeeded(providedConfig || {});
